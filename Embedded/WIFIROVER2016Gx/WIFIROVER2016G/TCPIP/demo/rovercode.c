...
void doclosedloop(void) {
    ...
    if (DesiredL == 0) {
        ...
        if (I16<-8191) I16 = -8191;
        errorL = (DesiredL + I16);
        if (errorL > 1000) errorL = 1000;
        else if (errorL<-1000) errorL = -1000;
        derrorL = LasterrorL - errorL;
        LasterrorL = errorL;
        erroriL += errorL / 50;
        if (erroriL > 1000) erroriL = 1000;
        else if (erroriL<-1000) erroriL = -1000;
    ...
    if (DesiredR == 0) {
        ...
        if (countup1) I16 = -CspeedA;
        else I16 = CspeedA;
        //      I16=(emf1v-emfoffset1);
        if (I16 > 8191) I16 = 8191;
        if (I16<-8191) I16 = -8191;
        errorR = (DesiredR + I16);
        if (errorR > 1000) errorR = 1000;
        else if (errorR<-1000) errorR = -1000;
        derrorR = LasterrorR - errorR;
        LasterrorR = errorR;
        erroriR += errorR / 50;
        if (erroriR > 1000) erroriR = 1000;
        else if (erroriR<-1000) erroriR = -1000;
...
void __attribute((interrupt(ipl3), vector(_ADC_VECTOR), nomips16)) _ADCInterrupt(void) {
    ...
    if (ToggleLine) {
        if (MotorPWMcount > 256) {
            if (setLineLed == 1) setLineLed = 2;
        } else {
            if (setLineLed == 0) setLineLed = 3;
        }
    }
    if (AD1CON2bits.BUFS) {
        if (ToggleLine) {
            if (setLineLed & 2) {
                setLineLed &= 1;
                if (setLineLed) {
                    LINEA = LINEAa - ADC1BUF0;
                    LINEB = LINEBa - ADC1BUF1;
                    LED2_IO = 1;
                    LED3_IO = 1;
                } else {
                    LINEAa = ADC1BUF0;
        ...
        if (ToggleLine) {
            if (setLineLed & 2) {
                setLineLed &= 1;
                if (setLineLed) {
                    LINEA = LINEAa - ADC1BUF8;
                    LINEB = LINEBa - ADC1BUF9;
                    LED2_IO = 1;
                    LED3_IO = 1;
                } else {
                    LINEAa = ADC1BUF8;
...
char yescomsdata(void) {
    if (comsdatatail != comsdatahead) return (1);
    else return (0);
}
...
void processcommand(void) // the main routine which processes commands
{
    ...
    {
        case CMDNULL:POSTTCPhead(0, CMDNULL);
            break;
        ...
        case CMDSETLED:if (commandlen == 1) //Set LEDs
            {
                if (nextcommand[1] & 2) LED1_IO = 0;
                else LED1_IO = 1;
                if (nextcommand[1] & 1) LED0_IO = 0;
                else LED0_IO = 1;
                POSTTCPhead(0, CMDSETLED);
                break;
            }
            break;
        case CMDGETCOUNT:POSTTCPhead(2, CMDGETCOUNT); // get internal counter
            POSTTCPchar((REQCOUNT >> 8) & 0xff);
        ...
        case CMDSETLINEFOLLOW: if (commandlen == 5) // set line following thrsholds and LEDS
            {
                if (nextcommand[1]& 1) LED2_IO = 1;
                else LED2_IO = 0;
                if (nextcommand[1]& 2) LED3_IO = 1;
                else LED3_IO = 0;
        ...
        case CMDRDgyro:
            if (commandlen == 0) //send gyro L3G4200D
            {
                ...
                if ((I2GET(0) & 0x8) == 0) {
                    ...
                    I2P();
                    POSTTCPchar(temparray[1]);
                    POSTTCPchar(temparray[0]);
                    POSTTCPchar(temparray[3]);
                    POSTTCPchar(temparray[2]);
                    POSTTCPchar(temparray[5]);
                    POSTTCPchar(temparray[4]);
                }
            }
            break;
        case CMDMagSTATUS:
        ...
        case CMDGetMotorSpeedWord: // set speed word ... return pos and speeds
            ...
            if (countup1) i = CspeedA;
            else i = -CspeedA;
            POSTTCPchar(i >> 8);
            POSTTCPchar(i);
            if (countup2) i = CspeedB;
            else i = -CspeedB;
            POSTTCPchar(i >> 8);
            POSTTCPchar(i);
            i = emfoffset1 - emf1v;
